      ...
      mouseMove: mxUtils.bind(this, function(sender, me) {
        if (
          ...
          if (
            ...
            if (
              Math.abs(start.point.x - me.getGraphX()) >
                tol * this.view.scale ||
...
Graph.prototype.createLayersDialog = function() {
  ...
  for (var i = 0; i < childCount; i++) {
    (function(layer) {
      ...
      span.appendChild(cb);
      var title =
        layer.value || (mxResources.get("background") || "Background");
...
Graph.prototype.connectVertex = function(
  ...
  var target =
    mxEvent.isControlDown(evt) && !forceClone
      ? null
    ...
    if (tmp != null) {
      ...
      for (var i = 0; i < attrs.length; i++) {
        if (
          ...
          tip +=
            (attrs[i].nodeName != "link" ? attrs[i].nodeName + ":" : "") +
  ...
  if (this.currentState != null) {
    ...
    if (
      ...
      this.arrowUp.style.left =
        Math.round(this.currentState.getCenterX() - this.triangleUp.width / 2) +
        "px";
      this.arrowUp.style.top =
        Math.round(bds.y - this.triangleUp.height) + "px";
      ...
      this.arrowRight.style.top =
        Math.round(
          this.currentState.getCenterY() - this.triangleRight.height / 2
        ) + "px";
      ...
      this.arrowLeft.style.left =
        Math.round(bds.x - this.triangleLeft.width) + "px";
      ...
      if (this.checkCollisions) {
        ...
        if (
          right != null &&
          !this.graph.model.isAncestor(right, this.currentState.cell)
        ) {
        ...
        if (
          left != null &&
          !this.graph.model.isAncestor(left, this.currentState.cell)
        ) {
        ...
        if (
          top != null &&
          !this.graph.model.isAncestor(top, this.currentState.cell)
        ) {
        ...
        if (
          bottom != null &&
          !this.graph.model.isAncestor(bottom, this.currentState.cell)
        ) {
...
mxStencilRegistry.getStencil = function(name) {
  ...
  if (
    result == null &&
    mxCellRenderer.prototype.defaultShapes[name] == null &&
...
if (typeof mxVertexHandler != "undefined") {
  (function() {
    ...
    Graph.prototype.getSvg = function(
      ...
      root.setAttribute(
        "width",
        Math.ceil(bounds.width * s) + 2 * border + "px"
      );
      root.setAttribute(
        "height",
        Math.ceil(bounds.height * s) + 2 * border + "px"
      );
    ...
    mxEdgeHandler.prototype.updateHint = function(me, point) {
      ...
      this.hideEdgeHintThread = window.setTimeout(
        mxUtils.bind(this, function() {
          if (this.hint != null) {
            this.hint.style.visibility = "hidden";
          }
    ...
    /**
		 ...
		 */
    HoverIcons.prototype.mainHandle = new mxImage(
      mxClient.IS_SVG
    ...
    HoverIcons.prototype.terminalHandle = new mxImage(
      mxClient.IS_SVG
        ...
        : IMAGE_PATH + "/handle-terminal.png",
      17,
      17
    );
    HoverIcons.prototype.secondaryHandle = new mxImage(
      mxClient.IS_SVG
        ...
        : IMAGE_PATH + "/handle-secondary.png",
      17,
      17
    );
    ...
    mxVertexHandler.prototype.redrawHandles = function() {
      ...
      if (this.state != null && this.linkHint != null) {
        ...
        this.linkHint.style.left =
          Math.round(rs.x + (rs.width - this.linkHint.clientWidth) / 2) + "px";
        this.linkHint.style.top =
          Math.round(
            bb.y +
              ...
              6 +
              this.state.view.graph.tolerance
          ) + "px";
      }
    };
    ...
    mxEdgeHandler.prototype.redrawHandles = function() {
      ...
      if (this.marker != null) {
        ...
        if (this.state != null && this.linkHint != null) {
          ...
          this.linkHint.style.left =
            Math.round(b.x + (b.width - this.linkHint.clientWidth) / 2) + "px";
          this.linkHint.style.top =
