...
io.on('connection', (socket) => {
  ...
  socket.on('login', async ({ name, room }) =>  {
      ...
      const responseUser = await axios.get('https://localhost:8080/modelo-c4/links/add/user/'+name);
      
      // consumir el endpoint para obtener la el proyecto por su codigo/llave
      // le paso el codigo de la sala
      // TODO: CAMBIAR A LA URL DE MI API
      const responseProject = await axios.get('https://localhost:8080/modelo-c4/cargar-diagrama/'+room);
  ...
  socket.on('save_component', async (  data ) => {
    ...
    if(findRoom(data.room)) {
        ...
        // TODO: CAMBIAR A LA URL DE MI API
        const response =  await axios.put('https://localhost:8080/modelo-c4/guardar-diagrama/'+data.room, body);
